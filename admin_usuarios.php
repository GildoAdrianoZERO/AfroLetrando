...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0"> <title>Super Admin | Gestão de Usuários</title>
    ...
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
            <div class="text-center md:text-left">
                <h1 class="text-2xl md:text-3xl font-bold text-stone-800"><i class="fas fa-user-shield text-orange-700 mr-2"></i> Gestão de Usuários</h1>
                <p class="text-stone-500 text-sm md:text-base">Controle total de permissões e acessos.</p>
            </div>
        ...
        <?php if (isset($_GET['msg'])): ?>
            <?php if ($_GET['msg'] == 'erro_proprio'): ?>
                <div class="bg-red-100 text-red-700 p-4 mb-6 rounded-lg font-bold border-l-4 border-red-500 text-center">Você não pode excluir a si mesmo!</div>
            <?php else: ?>
                <div class="bg-green-100 text-green-700 p-4 mb-6 rounded-lg font-bold border-l-4 border-green-500 text-center"><i class="fas fa-check-circle mr-2"></i> Ação realizada com sucesso!</div>
            <?php endif; ?>
        <?php endif; ?>
        ...
        <div class="bg-yellow-50 border border-yellow-200 rounded-xl shadow-sm mb-12 overflow-hidden">
            ...
            <div class="overflow-x-auto"> <table class="min-w-full whitespace-nowrap">
                    <tbody class="divide-y divide-yellow-200">
                        ...
                        <tr class="hover:bg-yellow-100/50">
                            ...
                            <td class="px-6 py-4 text-right space-x-2">
                                <a href="?acao=aprovar&id=<?php echo $user['id']; ?>" class="bg-green-600 text-white px-3 py-1 rounded text-sm hover:bg-green-700 shadow inline-block">Aprovar</a>
                                <a href="?acao=recusar&id=<?php echo $user['id']; ?>" class="bg-red-500 text-white px-3 py-1 rounded text-sm hover:bg-red-600 shadow inline-block" onclick="return confirm('Recusar este pedido?')">Recusar</a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
                    </tbody>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-stone-200 overflow-hidden">
            ...
            <div class="overflow-x-auto"> <table class="min-w-full divide-y divide-stone-100 whitespace-nowrap">
                        ...
                        <tr class="hover:bg-stone-50 transition">
                            ...
                            <td class="px-6 py-4 text-right">
                                <?php if ($user['nivel'] == 'editor'): ?>
                                    <a href="?acao=virar_admin&id=<?php echo $user['id']; ?>" class="text-purple-600 hover:text-purple-800 text-sm font-bold mr-4 inline-block mb-1 sm:mb-0" title="Promover a Admin">
                                        <i class="fas fa-arrow-up"></i> Promover
                                    </a>
                                <?php else: ?>
                                    <a href="?acao=virar_editor&id=<?php echo $user['id']; ?>" class="text-stone-500 hover:text-stone-800 text-sm font-bold mr-4 inline-block mb-1 sm:mb-0" title="Rebaixar a Editor">
                                ...
                                <a href="?acao=excluir&id=<?php echo $user['id']; ?>" 
                                   ...
                                   class="bg-red-100 text-red-600 hover:bg-red-600 hover:text-white px-3 py-1 rounded transition text-sm inline-block">
        ...
        <p class="mt-4 text-xs text-stone-400 text-center md:hidden">
            <i class="fas fa-hand-pointer mr-1"></i> Arraste a tabela para o lado para ver mais opções.
        </p>
